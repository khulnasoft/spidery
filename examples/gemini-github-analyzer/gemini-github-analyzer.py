...
if not spidery_api_key:
    print(f"{Colors.RED}{Emojis.WARNING} Warning: SPIDERY_API_KEY not found in environment variables{Colors.RESET}")
...
def poll_extraction_result(extraction_id, api_key, interval=2, max_attempts=15):
    """Poll Spidery API for extraction results with shorter intervals."""
...
def extract_github_profile(username, api_key):
    """Extract GitHub profile data using Spidery with optimized settings."""
    if not api_key:
        print(f"{Colors.RED}Error: Spidery API key is missing{Colors.RESET}")
        return None
...
def main():
    ...
    profile_data = extract_github_profile(username, spidery_api_key)
