...
if not spidery_api_key:
    print(f"{Colors.RED}Warning: SPIDERY_API_KEY not found in environment variables{Colors.RESET}")
...
def extract_company_info(urls, prompt, company, api_key):
    """Use requests to call Spidery's extract endpoint with selected URLs."""
    print(f"{Colors.YELLOW}Extracting structured data from the provided URLs using Spidery...{Colors.RESET}")
    ...
    try:
        ...
        return poll_spidery_result(extraction_id, api_key)
...
def poll_spidery_result(extraction_id, api_key, interval=10, max_attempts=60):
    """Poll Spidery API to get the extraction result."""
    url = f"https://api-spidery.khulnasoft.com/v1/extract/{extraction_id}"
    headers = {
        'Authorization': f'Bearer {api_key}'
    }
...
def main():
    ...
    data = extract_company_info(selected_urls, objective, company, spidery_api_key)
