...
def initialize_clients():
    ...
    spidery_client = SpideryApp(api_key=spidery_api_key)
    
    return openai_client, hubspot_client, spidery_client
...
# Scrape URL using Spidery
def scrape_url(spidery_client, url):
    try:
        return spidery_client.scrape_url(url, params={'formats': ['markdown']})
    except Exception as e:
...
# Main process
def main():
    openai_client, hubspot_client, spidery_client = initialize_clients()
    ...
    for company in companies:
        ...
        scrape_status = scrape_url(spidery_client, url)
