...
2. Build Docker images, and host it in your Docker Registry (replace the target registry with your own)
   1. API (which is also used as a worker image)
      1. ```bash
         docker build --no-cache --platform linux/amd64 -t ghcr.io/winkk-dev/spidery:latest ../../../apps/api
         docker push ghcr.io/winkk-dev/spidery:latest
         ```
   2. Playwright 
      1. ```bash
            docker build --no-cache --platform linux/amd64 -t ghcr.io/winkk-dev/spidery-playwright:latest ../../../apps/playwright-service
