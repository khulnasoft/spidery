...
/**
 * Configuration interface for SpideryApp.
 * @param apiKey - Optional API key for authentication.
 * @param apiUrl - Optional base URL of the API; defaults to 'https://api-spidery.khulnasoft.com'.
 */
export interface SpideryAppConfig {
  apiKey?: string | null;
  apiUrl?: string | null;
}

/**
 * Metadata for a Spidery document.
 * Includes various optional properties for document metadata.
 */
export interface SpideryDocumentMetadata {
  title?: string;
  description?: string;
  language?: string;
  keywords?: string;
...
/**
 * Document interface for Spidery.
 * Represents a document retrieved or processed by Spidery.
 */
export interface SpideryDocument<T = any, ActionsSchema extends (ActionsResult | never) = never> {
  url?: string;
  markdown?: string;
  html?: string;
  rawHtml?: string;
  ...
  json?: T;
  screenshot?: string;
  metadata?: SpideryDocumentMetadata;
 ...
 */
export interface ScrapeResponse<LLMResult = any, ActionsSchema extends (ActionsResult | never) = never> extends SpideryDocument<LLMResult, ActionsSchema> {
  success: true;
  warning?: string;
  error?: string;
}
...
export interface CrawlStatusResponse {
  ...
  expiresAt: Date;
  next?: string;
  data: SpideryDocument<undefined>[];
};
...
/**
 * Custom error class for Spidery.
 * Extends the built-in Error class to include a status code.
 */
export class SpideryError extends Error {
  statusCode: number;
  details?: any;
...
/**
 * Main class for interacting with the Spidery API.
 * Provides methods for scraping, searching, crawling, and mapping web content.
 */
export default class SpideryApp {
  public apiKey: string;
  public apiUrl: string;

  private isCloudService(url: string): boolean {
    return url.includes('api-spidery.khulnasoft.com');
  }

  /**
   * Initializes a new instance of the SpideryApp class.
   * @param config - Configuration options for the SpideryApp instance.
   */
  constructor({ apiKey = null, apiUrl = null }: SpideryAppConfig) {
    const baseUrl = apiUrl || "https://api-spidery.khulnasoft.com";
    
    if (this.isCloudService(baseUrl) && typeof apiKey !== "string") {
      throw new SpideryError("No API key provided", 401);
    }
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (responseData.success) {
          ...
          throw new SpideryError(`Failed to scrape URL. Error: ${responseData.error}`, response.status);
        }
      } else {
  ...
  /**
   * Searches using the Spidery API and optionally scrapes the results.
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (responseData.success) {
          return {
            success: true,
            data: responseData.data as SpideryDocument<any>[],
            warning: responseData.warning,
          };
        } else {
          throw new SpideryError(`Failed to search. Error: ${responseData.error}`, response.status);
        }
      } else {
      ...
      if (error.response?.data?.error) {
        throw new SpideryError(`Request failed with status code ${error.response.status}. Error: ${error.response.data.error} ${error.response.data.details ? ` - ${JSON.stringify(error.response.data.details)}` : ''}`, error.response.status);
      } else {
        throw new SpideryError(error.message, 500);
      }
    }
  ...
  /**
   * Checks the status of a crawl job using the Spidery API.
  ...
  async checkCrawlStatus(id?: string, getAllData = false, nextURL?: string, skip?: number, limit?: number): Promise<CrawlStatusResponse | ErrorResponse> {
    if (!id) {
      throw new SpideryError("No crawl ID provided", 400);
    }
  ...
  /**
   * Cancels a crawl job using the Spidery API.
   * @param id - The ID of the crawl operation.
  ...
  async crawlUrlAndWatch(
    ...
    throw new SpideryError("Crawl job failed to start", 400);
  }

  /**
   * Maps a URL using the Spidery API.
  ...
  /**
   * Initiates a batch scrape job for multiple URLs using the Spidery API.
  ...
  async batchScrapeUrlsAndWatch(
    ...
    throw new SpideryError("Batch scrape job failed to start", 400);
  }

  /**
   * Checks the status of a batch scrape job using the Spidery API.
  ...
  async checkBatchScrapeStatus(id?: string, getAllData = false, nextURL?: string, skip?: number, limit?: number): Promise<BatchScrapeStatusResponse | ErrorResponse> {
    if (!id) {
      throw new SpideryError("No batch scrape ID provided", 400);
    }
  ...
  /**
   * Extracts information from URLs using the Spidery API.
  ...
  async extract<T extends zt.ZodSchema = any>(urls: string[], params?: ExtractParams<T>): Promise<ExtractResponse<zt.infer<T>> | ErrorResponse> {
    ...
    try {
      ...
      throw new SpideryError("Invalid schema. Schema must be either a valid Zod schema or JSON schema object.", 400);
    }
    ...
    try {
        ...
        do {
          ...
          if (extractStatus.status === "completed") {
            if (extractStatus.success) {
              ...
              throw new SpideryError(`Failed to extract data. Error: ${extractStatus.error}`, statusResponse.status);
            }
          } else if (extractStatus.status === "failed" || extractStatus.status === "cancelled") {
            throw new SpideryError(`Extract job ${extractStatus.status}. Error: ${extractStatus.error}`, statusResponse.status);
          }
      ...
      throw new SpideryError(error.message, 500, error.response?.data?.details);
    }
    return { success: false, error: "Internal server error."};
  }

  /**
   * Initiates an asynchronous extract job for a URL using the Spidery API.
  ...
  async monitorJobStatus(
    ...
    try {
          ...
          let statusData = statusResponse.data;
            if (statusData.status === "completed") {
              if ("data" in statusData) {
                ...
                throw new SpideryError("Crawl job completed but no data was returned", 500);
              }
            } else if (
      ...
      throw new SpideryError(error, 500);
    }
  }
  ...
  async deepResearch(
    ...
    try {
      ...
      if (!response.id) {
        throw new SpideryError(`Failed to start research. No job ID returned.`, 500);
      }
  ...
  async checkDeepResearchStatus(id: string): Promise<DeepResearchStatusResponse | ErrorResponse> {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        throw new SpideryError("Deep research job not found", 404);
      } else {
  ...
  async generateLLMsText(url: string, params?: GenerateLLMsTextParams): Promise<GenerateLLMsTextStatusResponse | ErrorResponse> {
    try {
      ...
      if (!response.id) {
        throw new SpideryError(`Failed to start LLMs.txt generation. No job ID returned.`, 500);
      }
  ...
  async checkGenerateLLMsTextStatus(id: string): Promise<GenerateLLMsTextStatusResponse | ErrorResponse> {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        throw new SpideryError("LLMs.txt generation job not found", 404);
      } else {
...
interface CrawlWatcherEvents {
  document: CustomEvent<SpideryDocument<undefined>>,
  done: CustomEvent<{
...
export class CrawlWatcher extends TypedEventTarget<CrawlWatcherEvents> {
  ...
  public data: SpideryDocument<undefined>[];
  public status: CrawlStatusResponse["status"];
  public id: string;

  constructor(id: string, app: SpideryApp) {
    super();
    this.id = id;
