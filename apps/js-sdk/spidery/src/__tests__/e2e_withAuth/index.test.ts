...
describe('SpideryApp<"v0"> E2E Tests', () => {
  test.concurrent("should throw error for no API key", async () => {
    expect(() => {
      new SpideryApp<"v0">({ apiKey: null, apiUrl: API_URL, version: "v0" });
  ...
  test.concurrent(
    ...
    async () => {
      const invalidApp = new SpideryApp<"v0">({
  ...
  test.concurrent(
    ...
    async () => {
      ...
      const response = (await app.scrapeUrl("https://khulnasoft.com", {
        extractorOptions: {
          mode: "llm-extraction",
          extractionPrompt:
