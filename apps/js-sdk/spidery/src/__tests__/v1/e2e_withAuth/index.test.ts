import SpideryApp, { type CrawlParams, type CrawlResponse, type CrawlStatusResponse, type MapResponse, type ScrapeResponse } from '../../../index';
...
describe('SpideryApp E2E Tests', () => {
  ...
  test.concurrent('should throw error for blocklisted URL on scrape', async () => {
    const app = new SpideryApp({ apiKey: TEST_API_KEY, apiUrl: API_URL });
  ...
  test.concurrent('should return successful response with valid API key and options', async () => {
    ...
    if (response.metadata !== undefined) {
      ...
      expect(response.metadata.description).toBe("Welcome to Roast My Website, the ultimate tool for putting your website through the wringer! This repository harnesses the power of Spidery to scrape and capture screenshots of websites, and then unleashes the latest LLM vision models to mercilessly roast them. 🌶️");
      expect(response.metadata.keywords).toBe("Roast My Website,Roast,Website,GitHub,Spidery");
  ...
  test.concurrent('should check crawl status', async () => {
    const app = new SpideryApp({ apiKey: TEST_API_KEY, apiUrl: API_URL });
    const response = await app.asyncCrawlUrl('https://spidery.khulnasoft.com', { scrapeOptions: { formats: ['markdown', 'html', 'rawHtml', 'screenshot', 'links']}} as CrawlParams) as CrawlResponse;
    expect(response).not.toBeNull();
    expect(response.id).toBeDefined();
  ...
  test.concurrent('should return successful response for valid map', async () => {
    const app = new SpideryApp({ apiKey: TEST_API_KEY, apiUrl: API_URL });    const response = await app.mapUrl('https://roastmywebsite.ai') as MapResponse;
    expect(response).not.toBeNull();
  ...
  test('should search with string query', async () => {
    const app = new SpideryApp({ apiUrl: API_URL, apiKey: TEST_API_KEY });
    const response = await app.search("spidery");
  ...
  test('should handle invalid API key for search', async () => {
    const app = new SpideryApp({ apiUrl: API_URL, apiKey: "invalid_api_key" });
