...
use dotenvy::dotenv;
use serde_json::json;
use spidery::scrape::{ExtractOptions, ScrapeFormats, ScrapeOptions};
use spidery::{SpideryApp, SpideryError};
use std::env;
...
//     let app = SpideryApp::new_selfhosted(api_url, api_key).unwrap();
//     let blocklisted_url = "https://facebook.com/fake-test";
//     let result = app.scrape_url(blocklisted_url, None).await;

//     assert_matches!(
//         result,
//         Err(e) if e.to_string().contains("Spidery currently does not support social media scraping due to policy restrictions")
//     );
// }
...
async fn test_llm_extraction() {
    ...
    let result = app
        .scrape_url("https://khulnasoft.com", options)
        .await
        .unwrap();
...
fn test_api_key_requirements() {
    ...
    match (api_url.contains("api.spidery.khulnasoft.com"), api_key) {
        ...
        (true, None) => {
            ...
            assert!(matches!(
                result,
                Err(SpideryError::APIError(msg, _)) if msg == "Configuration"
            ));
        }
        (true, Some(key)) => {
            let result = SpideryApp::new_selfhosted(&api_url, Some(&key));
            assert!(result.is_ok());
        }
    }
}
