...
use error::SpideryAPIError;
pub use error::SpideryError;

#[derive(Clone, Debug)]
pub struct SpideryApp {
...
impl SpideryApp {
    ...
    async fn handle_response<'a, T: DeserializeOwned>(
        ...
        match &response {
            Ok(_) => response,
            Err(SpideryError::ResponseParseError(_))
            | Err(SpideryError::ResponseParseErrorText(_)) => {
                if is_success {
                    response
                } else {
                    Err(SpideryError::HttpRequestFailed(
                        action.as_ref().to_string(),
                        status.as_u16(),
