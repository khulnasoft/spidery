...
use crate::{
    ...
    SpideryApp, SpideryError, API_VERSION,
};
...
pub struct CrawlOptions {
    ...
    /// When using `SpideryApp::crawl_url`, this is how often the status of the job should be checked, in milliseconds. (default: `2000`)
    #[serde(skip)]
...
struct CrawlResponse {
    /// This will always be `true` due to `SpideryApp::handle_response`.
    /// No need to expose.
    success: bool,
...
impl SpideryApp {
    ...
    pub async fn crawl_url_async(
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| SpideryError::HttpError(format!("Crawling {:?}", url.as_ref()), e))?;
    ...
    pub async fn check_crawl_status(
        ...
        let response = self
            ...
            .await
            .map_err(|e| {
                SpideryError::HttpError(format!("Checking status of crawl {}", id.as_ref()), e)
            })?;
        ...
        loop {
            ...
            match status_data.status {
                ...
                CrawlStatusTypes::Failed => {
                    break Err(SpideryError::CrawlJobFailed(
