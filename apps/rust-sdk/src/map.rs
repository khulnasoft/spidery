...
use crate::{SpideryApp, SpideryError, API_VERSION};
...
impl SpideryApp {
    /// Returns links from a URL using the Spidery API.
    pub async fn map_url(
        &self,
        url: impl AsRef<str>,
        options: impl Into<Option<MapOptions>>,
    ) -> Result<Vec<String>, SpideryError> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| SpideryError::HttpError(format!("Mapping {:?}", url.as_ref()), e))?;
