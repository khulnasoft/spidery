...
function prepareOpenAIDoc(
  ...
  if (!extractionTarget) {
    return null;
    // throw new Error(
    //   `${mode} content is missing in the document. This is likely due to an error in the scraping process. Please try again or reach out to help@khulnasoft.com`
    // );
  }
...
export async function generateOpenAICompletions({
  ...
  return {
    ...
    warning:
      numTokens > maxTokens
        ? `Page was trimmed to fit the maximum token limit defined by the LLM model (Max: ${maxTokens} tokens, Attemped: ${numTokens} tokens). If results are not good, email us at help@khulnasoft.com so we can help you.`
        : undefined,
  };
}
