  ...
  try {
    ...
    if (result.success) {
      ...
      await updateExtract(job.data.extractId, {
        ...
        error:
          result.error ??
          "Unknown error, please contact help@spidery.com. Extract id: " +
            job.data.extractId,
      });
...
async function processJob(job: Job & { id: string }, token: string) {
  ...
  //     project_id: job.data.project_id,
  //     error:
  //       "URL is blocked. Suspecious activity detected. Please contact help@spidery.com if you believe this is an error.",
  //   };
  //   return data;
  // }
    ...
    await logJob(
      {
        ...
        message:
          typeof error === "string"
            ? error
            : (error.message ??
              "Something went wrong... Contact help@khulnasoft.com"),
        num_docs: 0,
        docs: [],
        time_taken: timeTakenInSeconds,
        team_id: job.data.team_id,
