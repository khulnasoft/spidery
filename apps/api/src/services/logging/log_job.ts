...
async function indexJob(job: SpideryJob): Promise<void> {
  try {
    ...
    const response = await fetch(
      `${process.env.FIRE_INDEX_SERVER_URL}/api/jobs`,
      {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
      ...
      logger.error(
        `Failed to send job to external server: ${response.status} ${response.statusText}`,
        {
          error: errorData,
          scrapeId: job.job_id,
        },
      ...
      logger.debug("Job sent to external server successfully!", {
        scrapeId: job.job_id,
      });
    }
  } catch (error) {
...
export async function logJob(job: SpideryJob, force: boolean = false) {
  try {
