...
import {
  SpideryCrawlResponse,
  SpideryCrawlStatusResponse,
  SpideryScrapeResponse,
} from "../../types";
...
describe("E2E Tests for Extract API Routes", () => {
  it.concurrent(
    "should return authors of blog posts on spidery.khulnasoft.com",
    async () => {
      const response = await request(TEST_URL)
        ...
        .send({
          urls: ["https://spidery.khulnasoft.com/*"],
  ...
  it.concurrent(
    "should return founders of spidery.khulnasoft.com (allowExternalLinks = true)",
    async () => {
      const response = await request(TEST_URL)
        .post("/v1/extract")
        .set("Authorization", `Bearer ${process.env.TEST_API_KEY}`)
  ...
  it.concurrent(
    "should return hiring opportunities on spidery.khulnasoft.com (allowExternalLinks = true)",
    async () => {
      const response = await request(TEST_URL)
        .post("/v1/extract")
        .set("Authorization", `Bearer ${process.env.TEST_API_KEY}`)
      ...
      for (const hiring of response.body.data?.items) {
        if (hiring.includes("Spidery Example Creator")) gotItRight++;
