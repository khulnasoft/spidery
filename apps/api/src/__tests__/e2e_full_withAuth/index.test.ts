...
describe("E2E Tests for API Routes", () => {
  ...
  describe("POST /v0/scrape", () => {
    ...
    it.concurrent(
      ...
      async () => {
        ...
        expect(response.body.data.metadata.description).toBe(
          "Welcome to Roast My Website, the ultimate tool for putting your website through the wringer! This repository harnesses the power of Spidery to scrape and capture screenshots of websites, and then unleashes the latest LLM vision models to mercilessly roast them. 🌶️",
        );
        expect(response.body.data.metadata.keywords).toBe(
          "Roast My Website,Roast,Website,GitHub,Spidery",
        );
    ...
    //     .set("Authorization", `Bearer ${process.env.TEST_API_KEY}`)
    //     .set("Content-Type", "application/json")
    //     .send({ url: "https://spidery.khulnasoft.com", pageOptions: { waitFor: 7000 } });
    ...
    //   expect(response.body.data).toHaveProperty("metadata");
    //   expect(response.body.data).not.toHaveProperty("html");
    //   expect(response.body.data.content).toContain("🔥 Spidery");
  ...
  describe("POST /v0/crawl", () => {
    ...
    it.concurrent(
      ...
      async () => {
        ...
        urls.forEach((url: string) => {
          expect(
            url.startsWith("https://www.khulnasoft.com/blog/"),
          ).toBeTruthy();
        });
    ...
    it.concurrent(
      ...
      async () => {
        ...
        urls.forEach((url: string) => {
          expect(
            url.startsWith("https://wwww.khulnasoft.com/blog/"),
          ).toBeFalsy();
        });
      },
      90000,
      ...
      async () => {
        ...
        expect(crawlData).toEqual(
          expect.arrayContaining([
            ...
            expect.objectContaining({
              url: expect.stringContaining("https://khulnasoft.com/pricing"),
            }),
            expect.objectContaining({
  ...
  describe("POST /v0/crawlWebsitePreview", () => {
    ...
    //   expect(response.statusCode).toBe(403);
    //   expect(response.body.error).toContain("Spidery currently does not support social media scraping due to policy restrictions. We're actively working on building support for it.");
    // });
    ...
    it.concurrent(
      ...
      async () => {
        const response = await request(TEST_URL)
          ...
          .set("Authorization", `Bearer ${process.env.TEST_API_KEY}`)
          .set("Content-Type", "application/json")
          .send({ url: "https://spidery.khulnasoft.com", timeout: 1000 });
    ...
    it.concurrent(
      ...
      async () => {
        const crawlResponse = await request(TEST_URL)
          ...
          .set("Authorization", `Bearer ${process.env.TEST_API_KEY}`)
          .set("Content-Type", "application/json")
          .send({ url: "https://khulnasoft.com/blog" });
        expect(crawlResponse.statusCode).toBe(200);
        ...
        const childrenLinks = completedResponse.body.data.filter(
          (doc) =>
            ...
            doc.metadata.sourceURL.includes("khulnasoft.com/blog"),
        );
