...
configDotenv();
const SPIDERY_API_URL = "http://127.0.0.1:3002";
const E2E_TEST_SERVER_URL = "http://spidery-e2e-test.vercel.app"; // @rafaelsideguide/spidery-e2e-test

describe("E2E Tests for v1 API Routes", () => {
  it.concurrent(
    "should return a successful response for a scrape with 403 page",
    async () => {
      const response: any = await request(SPIDERY_API_URL)
        .post("/v1/scrape")
  ...
  it.concurrent(
    ...
    async () => {
      ...
      expect(response.body.data.markdown).toContain(
        "This page is used for end-to-end (e2e) testing with Spidery.",
      );
      expect(response.body.data.markdown).toContain(
      ...
      ); // spidery.khulnasoft.com inside an iframe
      expect(response.body.data.markdown).toContain(
  ...
  it.concurrent(
    ...
    async () => {
      ...
      expect(response.body.data.html).toContain(
        '<p style="">This page is used for end-to-end (e2e) testing with Spidery.</p>',
      );
    },
    30000,
  );
  ...
  it.concurrent(
    ...
    async () => {
      ...
      expect(response.body.data.rawHtml).toContain(
        ">This page is used for end-to-end (e2e) testing with Spidery.</p>",
      );
  ...
  it.concurrent(
    ...
    async () => {
      ...
      const scrapeRequest = {
        url: E2E_TEST_SERVER_URL,
        headers: { "e2e-header-test": "spidery" },
      } as ScrapeRequest;
      ...
      expect(response.body.data.markdown).toContain("e2e-header-test: spidery");
    },
    30000,
  );
  ...
  it.concurrent(
    ...
    async () => {
      ...
      expect(response.body.data.markdown).not.toContain(
        "<p>This page is used for end-to-end (e2e) testing with Spidery.</p>",
      );
      expect(response.body.data.markdown).toContain(
        "Content with id #content-1",
      );
  ...
  it.concurrent(
    ...
    async () => {
      ...
      const responseNoParsePDF: any = await request(SPIDERY_API_URL)
        .post("/v1/scrape")
        .set("Authorization", `Bearer ${process.env.TEST_API_KEY}`)
        .set("Content-Type", "application/json")
        .send({
  ...
  it.concurrent(
    ...
    async () => {
      ...
      expect(response.body.data.actions.screenshots[0]).toContain(
        "https://service.spidery.khulnasoft.com/storage/v1/object/public/media/screenshot-",
      );
  ...
  it.concurrent(
    ...
    async () => {
      ...
      expect(response.body.data.actions.scrapes[0].url).toBe(
        "https://spidery-e2e-test.vercel.app/",
      );
