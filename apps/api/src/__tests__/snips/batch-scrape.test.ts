  ...
  it.concurrent(
    ...
    async () => {
      const response = await batchScrape({
        urls: ["http://spidery.khulnasoft.com"],
      });

      expect(response.body.data[0]).toHaveProperty("markdown");
      expect(response.body.data[0].markdown).toContain("Spidery");
    },
    30000,
  );
