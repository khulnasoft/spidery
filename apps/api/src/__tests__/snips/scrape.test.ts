...
describe("Scrape tests", () => {
  it.concurrent(
    ...
    async () => {
      ...
      // this test will fail if mock is bypassed with real data -- spidery.khulnasoft.com will never have
      // that as its actual markdown output

      const response = await scrape({
        url: "http://spidery.khulnasoft.com",
  ...
  it.concurrent(
    ...
    async () => {
      ...
      expect(response.markdown).toContain("Spidery");
    },
    30000,
  );
  ...
  if (!process.env.TEST_SUITE_SELF_HOSTED) {
      ...
      it.concurrent(
        ...
        async () => {
          ...
          expect(response.markdown).toMatch(
            /(\.g\.doubleclick\.net|amazon-adsystem\.com)\//,
          );
        },
        30000,
      );
    ...
    describe("PDF (f-e dependant)", () => {
      it.concurrent(
        "works for PDFs behind anti-bot",
        async () => {
          const response = await scrape({
          ...
          expect(response.markdown).toContain(
            "Robust adaptive beamforming based on jointly estimating covariance matrix",
          );
        },
        60000,
      );
