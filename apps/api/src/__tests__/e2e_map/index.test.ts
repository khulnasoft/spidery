...
describe("E2E Tests for Map API Routes", () => {
  it.concurrent(
    ...
    async () => {
      ...
      expect(response.body.links[0]).toContain(
        "spidery.khulnasoft.com/smart-crawl",
      );
    },
    60000,
  );
  ...
  it.concurrent(
    "(feat-subdomains) should return mapped links for spidery.khulnasoft.com with subdomains included",
    async () => {
      const response = await request(TEST_URL)
        .post("/v1/map")
        .set("Authorization", `Bearer ${process.env.TEST_API_KEY}`)
  ...
  it.concurrent(
    "(feat-sitemap-only) should return mapped links for spidery.khulnasoft.com with sitemap only",
    async () => {
      const response = await request(TEST_URL)
        .post("/v1/map")
        .set("Authorization", `Bearer ${process.env.TEST_API_KEY}`)
  ...
  it.concurrent(
    "(feat-limit) should return mapped links for spidery.khulnasoft.com with a limit",
    async () => {
      const response = await request(TEST_URL)
        .post("/v1/map")
        .set("Authorization", `Bearer ${process.env.TEST_API_KEY}`)
