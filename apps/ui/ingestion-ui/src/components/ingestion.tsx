...
export default function SpideryComponent() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const endpoint = `${SPIDERY_API_URL}/v0/${
      ...
      const response = await fetch(endpoint, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${SPIDERY_API_KEY}`,
      ...
      if (formData.crawlSubPages) {
        ...
        if (jobId) {
          const statusEndpoint = `${SPIDERY_API_URL}/v0/crawl/status/${jobId}`;
          let statusData: {
            status: string;
            data?: { url: string }[];
            current?: number;
  ...
  const handleScrapeSelected = async () => {
    ...
    for (const [index, url] of selectedUrls.entries()) {
      try {
        const response = await fetch(`${SPIDERY_API_URL}/v0/scrape`, {
          method: "POST",
          headers: {
            Authorization: `Bearer ${SPIDERY_API_KEY}`,
            "Content-Type": "application/json",
      ...
      <Card>
        <CardHeader className="flex items-start justify-between mb-0 pb-4">
          <CardTitle className="flex items-center justify-between w-full space-x-2">
            ...
            <a
              href="https://www.spidery.khulnasoft.com"
        ...
        <CardContent className="space-y-4">
          <form onSubmit={handleSubmit}>
            <div className="flex items-center space-x-2">
              <Input
                placeholder="https://www.spidery.khulnasoft.com/"
                className="flex-grow"
                name="url"
                value={formData.url}
